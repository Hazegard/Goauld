...
func run(domain dns.Name, localAddr *net.TCPAddr, remoteAddr net.Addr, pconn net.PacketConn) error {
	...
	var ln net.Listener
	// If localAddr is not nil, we listen on a local port.
	// Otherwise the client is listening to stdin, and writing to stdout
	// For used in ssh proxycommand for instance
	if localAddr != nil {
		var err error
	...
	if localAddr != nil {
		// IF the local address is not nil, we wait for a connection on the port
		for {
			local, err := ln.Accept()
			if err != nil {
			...
			go func() {
				defer local.Close()
				err := handle(local.(*net.TCPConn), local.(*net.TCPConn), sess, conn.GetConv())
				if err != nil {
		...
		// Is the local address nil, we read from stdin and write to stdout.
		err := handle(os.Stdin, os.Stdout, sess, conn.GetConv())
		if err != nil {
			return err
		}
...
func main() {
  ...
  %[1]s [-udp ADDR] DOMAIN [LOCALADDR]
  %[1]s If LOCALADDR is "-" or not provided, the client will listen on stdin and write to stdout, to be used in ssh proxycommand for instance.
...
`, os.Args[0])
	...
	if flag.Arg(1) == "-" || flag.NArg() == 1 {
		// if the listen address is "-" or if no listen address is given,
		// We use stdin/stdout to read/write traffic
		localAddr = nil
	} else {
		var err error
