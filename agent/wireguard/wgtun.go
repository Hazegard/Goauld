...
func CreateNetTUN(localAddresses, dnsServers []netip.Addr, mtu int) (tun.Device, *Net, error) {
	...
	dev.stack.SetTransportProtocolHandler(
		tcp.ProtocolNumber,
		tcp.NewForwarder(
			dev.stack,
			0,
			10000,
			dev.acceptTCP).HandlePacket)
	dev.stack.SetTransportProtocolHandler(
		udp.ProtocolNumber,
		udp.NewForwarder(
			dev.stack,
	...
	dev.stack.SetTransportProtocolHandler(icmp.ProtocolNumber4, ICMPHandler(dev.stack))
	...
	dev.stack.SetRouteTable([]tcpip.Route{
		{
...
func dumpPacket(pkt []byte) {
	if len(pkt) < 20 {
		fmt.Printf("  [short packet]\n")
	...
	switch pkt[0] >> 4 {
	case 4:
		hdr, err := goipv4.ParseHeader(pkt)
		if err != nil {
			fmt.Printf("  [invalid IPv4] %v\n", err)

			return
		}
		fmt.Printf("  IPv4 %s → %s proto=%d len=%d\n",
			hdr.Src, hdr.Dst, hdr.Protocol, hdr.TotalLen)
	case 6:
		fmt.Printf("  IPv6 packet %d bytes\n", len(pkt))
	default:
...
func (pc *PingConn) WriteTo(p []byte, addr net.Addr) (n int, err error) {
	...
	default:
		return 0, errors.New("ping write: wrong net.Addr type")
	}
	if !((na.Is4() && pc.laddr.addr.Is4()) || (na.Is6() && pc.laddr.addr.Is6())) {
		return 0, errors.New("ping write: mismatched protocols")
	}
