...
func handshakeControlProxy(rw io.ReadWriteCloser) (connTransport, error) {
	...
	if m.messageType != muxMsgHello {
		return nil, errors.New("mux reply not hello")
	}
	if v, err := m.readUint32(); err != nil || v != 4 {
		return nil, errors.New("mux reply hello has bad protocol version")
	}
	m, err = r.next()
	if err != nil {
		return nil, fmt.Errorf("error reading mux server proxy: %w", err)
	}
...
func (p controlMessage) readString() (string, error) {
	...
	b := p.body.Next(int(l))
	if len(b) != int(l) {
		return string(b), errors.New("EOF on string read")
	}
...
func (r controlReader) next() (*controlMessage, error) {
	p := &controlMessage{}
	var l uint32
	err := binary.Read(r.r, binary.BigEndian, &l)
	if err != nil {
		return nil, fmt.Errorf("error reading message length: %w", err)
	}
	_, err = io.CopyN(&p.body, r.r, int64(l))
	if err != nil {
		return nil, fmt.Errorf("error reading message payload: %w", err)
	}
